...
describe('View Snapshot Tests: All Pages', () => {
  ...
  it('should match snapshot for the expanded predictor badge page (/expanded-predictor-badge/new)', async () => {
    const response = await request(app).get('/expanded-predictor-badge/new').expect(200)
    expect(response.text).toMatchSnapshot()
  })
